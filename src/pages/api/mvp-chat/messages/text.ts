...
import { isSendMvpChatTextMessageRequestBody } from '../../../../server/mvp-chat/mvp-chat-endpoint-type-guards.utils';
import { MvpChatStorageReadError, appendMvpChatTextMessage } from '../../../../server/mvp-chat/mvp-chat-storage.utils';
...
export const POST = (async ({ request }) => {
  ...
  if (!isSendMvpChatTextMessageRequestBody(requestBody)) {
    return createJsonResponse(
      MVP_CHAT_ENDPOINT_ERROR_RESPONSES.invalidRequestBody,
      API_RESPONSE_STATUS_CODES.badRequest,
    );
  ...
  try {
    const message = await appendMvpChatTextMessage({
